---
title: createStream
description: createStream API reference
---

# createStream
**POST**  
**Service & Port:** config:80 -- e.g. http://localhost:80/createStream  
**Required fields:** `message_type`, `folder_name`  
`createStream` creates a `stream` configuration record that defines a data source 
a storage destination as well as parameters for the data lake that will be created.

## Local
  * Example `createStream` call body for creating a local data lake.  
    ```
    {
      "active": true,
      "message_type": "test-msg-local",
      "file_store_type_id": 1,
      "folder_name": "testFolderLocal",
      "partition_time_id": 1,
      "compression_type_id": 1
    }
    ```
  * Example `createStream` curl call for creating a local data lake.  
    ```
    curl --location --request POST 'http://localhost:80/createStream' \
    --header 'Content-Type: application/json' \
    --data-raw '{
      "active": true,
      "message_type": "test-msg-local",
      "file_store_type_id": 1,
      "folder_name": "testFolderLocal",
      "partition_time_id": 1,
      "compression_type_id": 1
    }'
    ```

## AWS
  * Example `createStream` call body for creating a data lake on AWS S3.  
    ```
    {
      "active": true,
      "message_type": "test-msg-aws",
      "file_store_type_id": 2,
      "region": "us-west-1",
      "bucket_name": "testBucketAWS",
      "folder_name": "testFolderAWS",
      "partition_time_id": 1,
      "compression_type_id": 1,
      "aws_access_key_id": "[aws_access_key_id]",
      "aws_secret_access_key": "[aws_secret_access_key]"
    }
    ```
  * Example `createStream` curl call for creating a data lake on AWS S3.  
    ```
    curl --location --request POST 'http://localhost:80/createStream' \
    --header 'Content-Type: application/json' \
    --data-raw '{
      "active": true,
      "message_type": "test-msg-aws",
      "file_store_type_id": 2,
      "region": "us-west-1",
      "bucket_name": "testBucketAWS",
      "folder_name": "testFolderAWS",
      "partition_time_id": 1,
      "compression_type_id": 1,
      "aws_access_key_id": "[aws_access_key_id]",
      "aws_secret_access_key": "[aws_secret_access_key]"
    }'
    ```

## GCP
  * Example `createStream` call body for creating a data lake on Google Cloud Storage.  
    ```
    {
      "active": true,
      "message_type": "test-msg-gcp",
      "file_store_type_id": 3,
      "bucket_name": "testBucketGCP",
      "folder_name": "testFolderGCP",
      "partition_time_id": 1,
      "compression_type_id": 1,
      "gcp_json_credentials": {"type": "service_account","project_id": "your_project","private_key_id": "aayourkeyid0f62a75","private_key": "-----BEGIN PRIVATE KEY-----\\nBQilFBKqXV/6DX8L/yourkeyiJR7QLhIpOpM3R\naE1/DrONSlAuKYhItMRTkowVSmSUNLLUs+88vzESGajofXYyRS3PlUXAZ/W4K/66\nTZNxtuLDF4CGuo2siLduthYrywKBgQC1Diu3lpunOMUVq9DBE9dLdoyt9PUpctCC\nCVGMi0i/hjJTv9rcFKREtdND/cM65uwml0/C0qD+Dj4kH7J3IYigHChpktCcux0j\\n-----END PRIVATE KEY-----\n","client_email": "rtdl-test@yoursa.iam.gserviceaccount.com","client_id": "112794475542667351829","auth_uri": "https://accounts.google.com/o/oauth2/auth","token_uri": "https://oauth2.googleapis.com/token","auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs","client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/rtdl-test%40yoursa.iam.gserviceaccount.com"}
    }
    ```
  * Example `createStream` curl call for creating a data lake on Google Cloud Storage.  
    ```
    curl --location --request POST 'http://localhost:80/createStream' \
    --header 'Content-Type: application/json' \
    --data-raw '{
      "active": true,
      "message_type": "test-msg-gcp",
      "file_store_type_id": 3,
      "bucket_name": "testBucketGCP",
      "folder_name": "testFolderGCP",
      "partition_time_id": 1,
      "compression_type_id": 1,
      "gcp_json_credentials": {"type": "service_account","project_id": "your_project","private_key_id": "aayourkeyid0f62a75","private_key": "-----BEGIN PRIVATE KEY-----\\nBQilFBKqXV/6DX8L/yourkeyiJR7QLhIpOpM3R\naE1/DrONSlAuKYhItMRTkowVSmSUNLLUs+88vzESGajofXYyRS3PlUXAZ/W4K/66\nTZNxtuLDF4CGuo2siLduthYrywKBgQC1Diu3lpunOMUVq9DBE9dLdoyt9PUpctCC\nCVGMi0i/hjJTv9rcFKREtdND/cM65uwml0/C0qD+Dj4kH7J3IYigHChpktCcux0j\\n-----END PRIVATE KEY-----\n","client_email": "rtdl-test@yoursa.iam.gserviceaccount.com","client_id": "112794475542667351829","auth_uri": "https://accounts.google.com/o/oauth2/auth","token_uri": "https://oauth2.googleapis.com/token","auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs","client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/rtdl-test%40yoursa.iam.gserviceaccount.com"}
    }'
    ```

## Azure
  * Example `createStream` call body for creating a data lake on Azure Blob Storage.  
    ```
    {
      "active": true,
      "message_type": "test-msg-azure",
      "file_store_type_id": 4,
      "bucket_name": "testBucketAzure",
      "folder_name": "testFolderAzure",
      "partition_time_id": 1,
      "compression_type_id": 1,
      "azure_storage_account_name": "[azure_storage_account_name]",
      "azure_storage_access_key": "[azure_storage_access_key]"
    }
    ```
  * Example `createStream` curl call for creating a data lake on Azure Blob Storage.  
    ```
    curl --location --request POST 'http://localhost:80/createStream' \
    --header 'Content-Type: application/json' \
    --data-raw '{
      "active": true,
      "message_type": "test-msg-azure",
      "file_store_type_id": 4,
      "bucket_name": "testBucketAzure",
      "folder_name": "testFolderAzure",
      "partition_time_id": 1,
      "compression_type_id": 1,
      "azure_storage_account_name": "[azure_storage_account_name]",
      "azure_storage_access_key": "[azure_storage_access_key]"
    }'
    ```

## All fields
    * Example `createStream` call body with all fields.  
    ```
    {
      "stream_alt_id": "abcdef-",
      "active": true,
      "message_type": "test-msg-",
      "file_store_type_id": 1,
      "region": "us-west1",
      "bucket_name": "testBucket",
      "folder_name": "testFolder",
      "partition_time_id": 1,
      "compression_type_id": 1,
      "aws_access_key_id": "test",
      "aws_secret_access_key": "test",
      "gcp_json_credentials": {"type": "service_account","project_id": "your_project","private_key_id": "aayourkeyid0f62a75","private_key": "-----BEGIN PRIVATE KEY-----\\nBQilFBKqXV/6DX8L/yourkeyiJR7QLhIpOpM3R\naE1/DrONSlAuKYhItMRTkowVSmSUNLLUs+88vzESGajofXYyRS3PlUXAZ/W4K/66\nTZNxtuLDF4CGuo2siLduthYrywKBgQC1Diu3lpunOMUVq9DBE9dLdoyt9PUpctCC\nCVGMi0i/hjJTv9rcFKREtdND/cM65uwml0/C0qD+Dj4kH7J3IYigHChpktCcux0j\\n-----END PRIVATE KEY-----\n","client_email": "rtdl-test@yoursa.iam.gserviceaccount.com","client_id": "112794475542667351829","auth_uri": "https://accounts.google.com/o/oauth2/auth","token_uri": "https://oauth2.googleapis.com/token","auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs","client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/rtdl-test%40yoursa.iam.gserviceaccount.com"},
      "azure_storage_account_name": "[azure_storage_account_name]",
      "azure_storage_access_key": "[azure_storage_access_key]"
    }
    ```
