---
title: updateStream
description: updateStream API reference
---

# updateStream
**PUT**  
**Service & Port:** config:80 -- e.g. http://localhost:80/updateStream  
**Required fields:** all fields required (any missing fields will be replaced 
with NULL values)  
`updateStream` updates a `stream` configuration record.
  * Example `updateStream` call body.  
    ```
    {
      "stream_id": "[stream_id]",
    ...
    ```
  * Example `updateStream` curl call.  
    ```
    curl --location --request PUT 'http://localhost:80/updateStream' \
