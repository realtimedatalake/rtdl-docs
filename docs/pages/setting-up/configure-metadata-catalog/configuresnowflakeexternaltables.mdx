...
[Snowflake External Tables](https://docs.snowflake.com/en/user-guide/tables-external-intro.html) support only works with AWS S3 storage for now. It also follows a non-standard congifuration path for now. It requires updates to the `docker-compose.yml` file. This method of configuration is temporary. We will be moving to a more standard method in the future.
